...
### T17 — English-only repo + 0.1.1 release · Status: IN PROGRESS(2026-09-05) · Depends on: T15
...
- Criteria: [x] zero Hangul characters in tracked files (`git grep -P '[\x{AC00}-\x{D7AF}]'` is empty) [x] CLAUDE.md, WORKFLOW and SPEC §7 record the English-only rule [x] release-gate wording says "maintainer approval"; the agent's HITL gate stays "human approval", and DESIGN §10 / WORKFLOW §4 state explicitly that installers are never asked for approval [x] check passes (208 passed, 100% coverage; only test fixtures and two smoke prompts changed in code, no logic) [ ] version `0.1.1` → PR → tag `v0.1.1` → TestPyPI → maintainer approval → PyPI, so the PyPI project page shows the new README
